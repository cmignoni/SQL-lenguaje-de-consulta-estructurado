...
El lenguaje estructurado de consultas (SQL) es un lenguaje de base de datos estándar, utilizado por la mayoría de los sistemas de gestión de bases de datos relacionales u objeto-relacionales.

Es un lenguaje declarativo en el que las órdenes especifican cual debe ser el resultado y no la manera de conseguirlo (como ocurre en los lenguajes imperativos o procedimentales). Al ser declarativo es muy sencillo su aprendizaje debido a que permiten escribir las órdenes, utilizando palabras clave (en inglés), como si fueran frases en las que se especifica que es lo que se quiere obtener; aunque a veces es dificil de comprender por el alto grado de abstracción. Por ejemplo:  
...
Esta sentencia devuelve el nombre de aquellos empleados que pertenecen al departamento producción (*prod*) y los presenta ordenados por nombre.

Debido a su eficiencia se ha convertido en un estándar para las bases de datos relacionales, de hecho, el gran éxito del modelo de base de datos relacional se debe en parte a la utilización de un lenguaje como SQL. Sobre la base común del lenguje se han desarrollado diversas versiones ampliadas como las de Oracle o la de Microsoft SQL server. Los comandos que contiene permiten realizar diversos tipos de tareas:
...
**PROC SQL** es la sentencia SAS que permite ejecutar las estructuras del lenguaje SQL encerrando las cláusulas del mismo.
...
Los resultados que produce la consulta anterior se observe en la siguiente imagen que muestra una ventana output de SAS

```{r, echo=FALSE, fig.align='center'}
knitr::include_graphics("g:/Mi unidad/SQL lenguaje de consulta estructurado/Material en RMarkDown/Image/img_001.1.png")
```
...
```{r, echo=FALSE, fig.align='center'}
knitr::include_graphics("g:/Mi unidad/SQL lenguaje de consulta estructurado/Material en RMarkDown/Image/img_003.png")
```
...
Dentro de la instrucción *SELECT FROM* la cláusula *WHERE* es opcional, si se incluye en una consulta debe ir siempre seguido a la cláusula FROM. El siguiente código SAS utiliza la clausula *WHERE* para seleccionar desde un conjunto de datos llamado "mundo" solo las filas que corresponden a los países que practican la religión "musulmana".
...
\begin{tabular}{ll}
\textbf{Avg} promedio de valores & \textbf{NMiss} número de valores faltantes \\ 
\textbf{Count} el número de valores que no faltan & \textbf{Prt} probabilidad de un mayor valor absoluto de t de Student \\
\textbf{CSS} suma corregida de cuadrados & \textbf{Range} rango de valores \\
\textbf{CV} Coeficiente de variación &	\textbf{Std} Desviación estándar \\
\textbf{Freq} (igual que Count) &	\textbf{StdErr} Error estándar de la media \\
\textbf{Max} Valor máximo &	\textbf{Sum} Suma de valores \\
\textbf{Mean} (igual que Avg) & \textbf{T} El valor t de Student para comprobar \\ 
& que la media de la población es 0 \\
\textbf{Min} Valor mínimo &	\textbf{USS} suma de cuadrados sin corregir \\
\textbf{N} (igual que Count) &	\textbf{Var} Variancia \\
...
```{r, echo=FALSE}
knitr::include_graphics("g:/Mi unidad/SQL lenguaje de consulta estructurado/Material en RMarkDown/Image/img_100.png")
```
...
La consulta anterior devuelve una lista de todos los valores únicos de religión.
No realiza un conteo, solo muestra una lista. Se pueden colocar varias columnas después del operador DISTINCT y se obtendrá una lista de todas las combinaciones de las columnas. Por ejemplo, la siguiente consulta devuelve los valores de las combinaciones únicas de marca y origen.
