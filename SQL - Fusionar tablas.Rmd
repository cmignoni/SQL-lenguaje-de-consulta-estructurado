...
Primero se definien dos conjuntos de datos que serán la base para los ejemplos posteriores. Por un lado, la tabla *Empleados* que almacena una lista de empleados y el id del departamento al quepertenecen:

**Empleados**

```{r, echo=F, eval= TRUE }
df1 <- read.csv("Datos/departamentos.csv")
```
...
```{r, echo=F, eval=TRUE}
df2 <- read.csv("Datos/empleados.csv")
```
...
Por ejemplo, si se quiere listar los empleados e indicar el nombre del departamento al que pertenecen, se reliza lo siguiente:
...
**Unión derecha**: devuelve todas las filas de la tabla derecha y cualquier fila coincidentes de la tabla izquierda. Si no existe ninguna coincidencia para alguna de las filas de la tabla de la izquierda, de igual forma todos los resultados de la primera tabla se muestran.
